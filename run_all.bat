...
echo [1/6] Launching Eureka Server (Port 8761)...
cd eureka-server
start "Eureka Server" cmd /k ".\mvnw.cmd spring-boot:run"
cd ..
timeout /t 5

echo [2/6] Launching User Service (Port 8811)...
cd user-service
start "User Service" cmd /k ".\mvnw.cmd spring-boot:run"
cd ..
timeout /t 5

echo [3/6] Launching Product Catalog Service (Port 8810)...
cd product-catalog-service
start "Product Catalog Service" cmd /k ".\mvnw.cmd spring-boot:run"
cd ..
timeout /t 5

echo [4/6] Launching Product Recommendation Service (Port 8812)...
